def create_video_config(clips, video_count, period, game):
    config = {}
    
    config['title'] = create_video_title(clips, video_count, period, game)
...
def create_video_title(clips, video_count, period, game):
    if period == 'day':
...
def create_video_description(clips):
    ...
    for clip in clips:
        ...
        description += '\n' + timestamp + ' ' + clean_title(clip['channel']) + ': '+  clean_title(clip['title']) + '\n' + 'https://clips.twitch.tv/' + clip['slug'] + '\n'
